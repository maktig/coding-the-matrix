...
three_digits_set = { (base**0*i)+(base**1*j)+(base**2*k) for i in digits for j in digits for k in digits }
...
L2 = [3,4,2,1] # <-- same len(L2) == len(list(set(L2))) but L2 != list(set(L2))

## Task 18
odd_num_list_range = {i for i in set(range(1,100,2))}
...
base = 10
digits = set(range(10))
representation_dict = { (base**0*i)+(base**1*j)+(base**2*k):[k,j,i] for i in digits for j in digits for k in digits }
